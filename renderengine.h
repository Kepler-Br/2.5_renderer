...
class RenderEngine
{
    ...
    void renderMap()
    {
        const glm::vec2 bias = glm::vec2(200.0f, 300.0f);
        constexpr float lookLength = 20.0f;
        window.setColor(glm::vec3(1.0f));
        window.square(glm::vec2(player.position)+bias-glm::vec2(3.0f), glm::vec2(6.0f));
        ...
        window.line(glm::vec2(player.position)+bias, glm::vec2(player.position.x + cos(player.angle)*lookLength, player.position.y + sin(player.angle)*lookLength)+bias);
        ...
        //            }
        //        }
        const Sector &currentSector = sectors[player.lastSector];
        ...
        for(int i = 0; i < currentSector.numWalls; i++)
        {
            ...
            if(currentWall.nextSectorIndex == -1)
                ...
                window.setColor(glm::vec3(1.0f, 0.0f, 0.0f));
            window.line(currentWall.point + bias, nextWall.point + bias);
    ...
    void render3D()
    {
        ...
        float aspectRatio = window.getGeometry().x/window.getGeometry().y;
        Sector currentSector = sectors[player.lastSector];
        Wall currentWall = walls[currentSector.startWall];

        window.setColor(glm::vec3(1.0f));
        window.square(glm::vec2(0.0f)+bias-glm::vec2(3.0f), glm::vec2(6.0f));

        window.line(glm::vec2(0.0f)+bias, glm::vec2(0.0f, 20.0f)+bias);
